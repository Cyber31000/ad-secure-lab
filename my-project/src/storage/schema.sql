...
CREATE UNIQUE INDEX IF NOT EXISTS idx_album_group_position ON album (group_date, position);
CREATE INDEX IF NOT EXISTS idx_album_group_date ON album (group_date DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_photo_album_path ON photo (album_id, relative_path);
-- I-3: one photo per slot within an album.
CREATE UNIQUE INDEX IF NOT EXISTS idx_photo_album_position ON photo (album_id, position);
...
CREATE VIEW IF NOT EXISTS album_main_page AS
SELECT
    a.id,
    a.name,
    a.group_date,
